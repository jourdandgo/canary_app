...
# --- 1. DATA LAYER: Biologically Grounded Data Generation ---
def generate_robust_poultry_dataset(days_to_simulate=500):
    """
    Generates data where 'Healthy' is tied to 'Optimal Consumption' and 'Normal Behavior'.
    Distress includes consumption drops AND behavioral anomalies (Droppings/Wings).
    """
    np.random.seed(42)
    random.seed(42)
    data = []
    ...
    current_bird_age = {zone: random.randint(1, 70) for zone in zones}
    current_bird_count = {zone: 10000 - (current_bird_age[zone] * 6) for zone in zones}
    cumulative_stress = {zone: 0.0 for zone in zones}
    
    for day_offset in range(days_to_simulate):
        current_date = start_date + timedelta(days=day_offset)
        base_temp = 27.0 + np.sin(day_offset / 15.0) * 4.0
        daily_max_temp_c = np.random.normal(base_temp, 1.5)
        daily_avg_humidity = np.random.normal(65, 5.0)
            
        for zone in zones:
            current_bird_age[zone] += 1
            if current_bird_age[zone] > 70:
                current_bird_age[zone] = 1 
                current_bird_count[zone] = 10000
                cumulative_stress[zone] = 0.0
            
            age = current_bird_age[zone]
            # BIOLOGICAL TARGETS (Healthy birds MUST reach these for growth)
            target_feed = (age * 4.8) + 20
            target_water = target_feed * 2.1
            
            # Stress Logic: Environmental factors push health down
            daily_stress = max(0, (daily_max_temp_c - 28.5)) * 2.5 + max(0, (daily_avg_humidity - 70)) * 1.0
            cumulative_stress[zone] = max(0, cumulative_stress[zone] * 0.70 + daily_stress)
            if random.random() < 0.02: cumulative_stress[zone] += 25.0 

            status = 'At_Risk' if cumulative_stress[zone] > 20.0 else 'Healthy'

            if status == 'Healthy':
                # Optimal consumption preserves health
                feed = np.random.normal(target_feed, target_feed * 0.03)
                water = np.random.normal(target_water, target_water * 0.03)
                wing_spread = np.random.normal(5, 2)
                droppings_score = np.random.normal(1.5, 0.5)
                mortality = max(0, int(np.random.normal(3, 1)))
            else:
                # SICK BIRDS: Consumption drops (Indicator)
                drop_factor = random.uniform(0.5, 0.75)
                feed = np.random.normal(target_feed * drop_factor, target_feed * 0.1)
                water = np.random.normal(target_water * drop_factor, target_water * 0.1)
                wing_spread = np.random.normal(45, 12)
                droppings_score = np.random.normal(7.5, 1.5)
                mortality = max(0, int(np.random.normal(40, 15)))

            current_bird_count[zone] -= min(mortality, current_bird_count[zone])
            
            data.append({
                'Date': current_date, 'Zone_ID': zone, 'Bird_Age_Days': age,
                'Total_Alive_Birds': current_bird_count[zone],
                'Max_Temperature_C': daily_max_temp_c, 'Avg_Humidity_Percent': daily_avg_humidity,
                'Avg_Water_Intake_ml': water, 'Avg_Feed_Intake_g': feed,
                'Wing_Spreading_Percent': wing_spread, 'Droppings_Abnormality_Score': droppings_score,
...
# --- 2. MODEL LAYER ---
df = generate_robust_poultry_dataset(500)
...
# Early Warning Logic (Predict T+1)
df['Target_Status'] = df.groupby('Zone_ID')['Health_Status'].shift(-1)
df_clean = df.dropna().copy()
df_ml = pd.get_dummies(df_clean, columns=['Zone_ID'], drop_first=True, dtype=int)

# Select features (order matters!)
X = df_ml.drop(['Date', 'Target_Status', 'Health_Status'], axis=1)
le = LabelEncoder()
y = le.fit_transform(df_ml['Target_Status'])
...
# --- 3. EXPORT ARTIFACTS ---
with open('random_forest_model_retrained.pkl', 'wb') as f:
    pickle.dump(model, f)
with open('X_train_data_retrained.pkl', 'wb') as f:
    pickle.dump(X_train, f)
with open('label_encoder.pkl', 'wb') as f:
    pickle.dump(le, f)
df_clean.to_csv('broiler_health_noisy_dataset.csv', index=False)
...
print("\nSuccess: Robust app.py generated. Column mismatch and matching logic fixed.")
print("Predictive Accuracy (T+1): {:.2%}".format(accuracy_score(y_test, model.predict(X_test))))
